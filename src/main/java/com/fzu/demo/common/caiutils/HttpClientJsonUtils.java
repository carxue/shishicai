...
public class HttpClientJsonUtils {
	...
	static{
		Integer[] a = {1,2,3};Integer[] b = {1,2,4};Integer[] c = {1,2,5};Integer[] d = {1,3,4};Integer[] e = {1,3,5};
		Integer[] f = {1,4,5};Integer[] g = {2,3,4};Integer[] h = {2,3,5};Integer[] i = {2,4,5};Integer[] j = {3,4,5};
		thirdTypeList.add(a);thirdTypeList.add(b);thirdTypeList.add(c);thirdTypeList.add(d);thirdTypeList.add(e);
		thirdTypeList.add(f);thirdTypeList.add(g);thirdTypeList.add(h);thirdTypeList.add(i);thirdTypeList.add(j);
	}
	...
	public static String postJson(String httpUrl, String date) {
		...
		try {
			...
			connection.setRequestProperty("Content-Type", FORM_HEADER_JSON.toString());
...
/*	public static String doPost(String url, Object obj) {
		String form = object2HttpFormString(url, obj);
		try {
			return postForm(url, form);
		} catch (IOException e) {
			LOG.error("post请求发生异常", e);
		}
		return "";
	}*/
...
/*	@SuppressWarnings("unchecked")
	private static String object2HttpFormString(String url, Object obj) {
		return UriComponentsBuilder.fromHttpUrl(url)
				.queryParams((new ObjectMapper()).enable(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY)
						.convertValue(obj, LinkedMultiValueMap.class))
				.build().toUri().toString().replace(url + "?", "");
	}*/
...
//		getEveryDayData(20181121);
	...
	public static List<CqTimeLottery> getEveryDayData(int selectDay){
        ...
        List<TimeRes> list = gson.fromJson(result, new TypeToken<List<TimeRes>>() {}.getType());
        for(TimeRes res:list){
	...
	public static void test(Integer Date) throws IOException {
		...
		List<TimeRes> timeList = gson.fromJson(resultJson, new TypeToken<List<TimeRes>>(){}.getType());
		for(TimeRes time:timeList){
			String number = time.getNum();
			if(StringUtils.isNotBlank(number)){
				System.out.println(time.getNo()+":"+number+" : "+getThirdNumType(number,1)+" : "+getThirdNumType(number,2)+" : "+getThirdNumType(number,3)+" : "+getFiveNumType(number));
			}
		}
	}
